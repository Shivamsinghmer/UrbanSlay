      ...
      <nav
        ...
        <div
          ...
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className="nav-card select-none relative flex flex-col gap-3 p-[16px_20px] rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-[1_1_auto] h-auto md:h-auto md:flex-[1_1_auto]"
              ref={setCardRef(idx)}
              style={{ backgroundColor: item.bgColor, color: item.textColor }}>
              <div
                className="nav-card-label font-semibold tracking-[-0.5px] text-[20px] md:text-[24px] mb-1">
                {item.label}
              </div>
